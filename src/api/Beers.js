...
export default function Beers() {
  ...
  const [beers, setData] = useState([{}]);                //Ici on déclare un useState avec Hooks
  const axios = require('axios');                         // On déclare axios pour pouvoir l'utiliser

  useEffect(() => {                                       // La fonction hook Effect appeler
    axios.get('https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=Beer', {  //ici on fait un appel get à l'API
    })
  ...
  console.log(beers);                                   //On affiche la liste dans un JSON 

  if (!beers) return null;                              //si aucun punch est retourner on retourne null
