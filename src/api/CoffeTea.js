...
export default function CoffeeTea() {
  ...
  const [coffee, setData] = useState([{}]);             //Ici on déclare un useState
  const axios = require('axios');                       // On déclare axios pour pouvoir l'utiliser

  useEffect(() => {                                     // hook Effect appeler 
    axios.get('https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=Coffee / Tea', {  //ici on fait un appel get à l'API
    })
  ...
  console.log(coffee);                                  //On affiche la liste dans un JSON 

  if (!coffee) return null;                            //si aucun punch est retourner on retourne null

  /*
  Ici on appel window qui représente la fenêtre Coffe Tea
