...
export default function HomeMadeLiqueur() {
    ...
    const [homemade, setData] = useState([{}]);                   //Ici on déclare un useState
    const axios = require('axios');                               // On déclare axios pour pouvoir l'utiliser 

    useEffect(() => {                                            // hook Effect appeler 
        axios.get('https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=Homemade Liqueur', {  //ici on fait un appel get à l'API
        })
    ...
    console.log(homemade);                                       //On affiche la liste dans un JSON

    if (!homemade) return null;                                 //si aucun punch est retourner on retourne null
    /*
    Ici on appel window qui représente la fenêtre HomeMade Liquor
