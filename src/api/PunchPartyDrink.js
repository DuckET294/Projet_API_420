...
export default function PunchPartyDrink() {
    ...
    const [punch, setData] = useState([{}]);                //Ici on déclare un useState
    const axios = require('axios');                         // On déclare axios pour pouvoir l'utiliser    

    useEffect(() => {                                       // hook Effect appeler

        axios.get('https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=Punch / Party Drink', {  //ici on fait un appel get à l'API avec axios
        })
    ...
    console.log(punch);                                     //On affiche la liste dans un JSON 

    if (!punch) return null;                                //si aucun punch est retourner on retourne null
